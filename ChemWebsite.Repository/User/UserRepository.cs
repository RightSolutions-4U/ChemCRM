...
{
    public class UserRepository : GenericRepository<User, ChemWebsiteDbContext>,
          IUserRepository
    {
        ...
        private async Task<List<AppClaimDto>> GetUserAndRoleClaims(User appUser)
        {
            ...
            foreach (var pageAction in pageActions)
            {
                ...
                {
                    ClaimType = $"{pageAction.Page.Name.ToLower().Replace(" ", "_")}_{pageAction.Action.Name.Trim().ToLower().Replace(" ", "_")}".Trim(),
                    ClaimValue = "false"
                };
